...
public:
  ...
  typename R::Ref create_request(P params)
  {
    ...
    constexpr bool enable_bypass_freezing = requires(typename R::Ref r, P p) {
      { p->want_bypass_freezing() } -> std::same_as<bool>;
      r->mark_want_bypass_freezing(true);
    };
    ...
    if constexpr (enable_bypass_freezing) {
      retval->mark_want_bypass_freezing(params->want_bypass_freezing());
    }
    return retval;
  }
};
