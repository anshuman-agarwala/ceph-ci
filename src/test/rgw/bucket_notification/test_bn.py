...
class KafkaReceiver(object):
    ...
    def __init__(self, topic, security_type):
        ...
        if remaining_retries == 0:
            raise Exception('kafka consumer on topic %s failed to connect to broker - no retries left', topic)
...
def create_kafka_receiver_thread(topic, security_type='PLAINTEXT'):
    ...
    ret = producer.send(topic, value=b'ping')
    log.info('pinged Kafka consumer on topic: %s. result: %s', receiver.topic, str(ret))
    retries = 0
