...
static auto transform_old_authinfo(std::unique_ptr<rgw::sal::User> user,
                                   std::optional<RGWAccountInfo> account,
                                   std::vector<IAM::Policy> policies)
  -> std::unique_ptr<rgw::auth::Identity>
{
  ...
  public:
    DummyIdentityApplier(std::unique_ptr<rgw::sal::User> user,
                         std::optional<RGWAccountInfo> account,
                         std::vector<IAM::Policy> policies)
      : id((user)?user->get_info().user_id:rgw_user("dummyidentity")),
        display_name((user)?user->get_info().display_name:""),
        path((user)?user->get_info().path:""),
        is_admin((user)?user->get_info().admin:false),
        type((user)?user->get_info().type:TYPE_RGW),
        muser(std::move(user)),
  ...
  return std::make_unique<DummyIdentityApplier>(
      std::move(user), std::move(account), std::move(policies));
}
  ...
  -> tl::expected<std::unique_ptr<Identity>, int>
{
  ...
  return transform_old_authinfo(std::unique_ptr<rgw::sal::User>(user), std::move(account), std::move(policies));
}
