  ...
  -> std::unique_ptr<rgw::auth::Identity>
{
  ...
  public:
    DummyIdentityApplier(std::unique_ptr<rgw::sal::User> user,
                         std::optional<RGWAccountInfo> account,
                         std::vector<IAM::Policy> policies)
      : id(user->get_info().user_id),
        display_name(user->get_info().display_name),
        path(user->get_info().path),
        is_admin(user->get_info().admin),
        type(user->get_info().type),
        muser(std::move(user)),
  ...
  -> tl::expected<std::unique_ptr<Identity>, int>
{
  ...
  return transform_old_authinfo(std::move(user), std::move(account), std::move(policies));
}
