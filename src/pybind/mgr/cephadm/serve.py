    ...
    def _refresh_hosts_and_daemons(self) -> None:
        ...
        def refresh(host: str) -> None:
            ...
            if self.mgr.cache.host_needs_custom_logrotate_file(host):
                for logrotate_type in [LogrotateConfigType.cephadm, LogrotateConfigType.cluster]:
                    if self.mgr.get_store(f'{str(logrotate_type)}_logrotate_config'):
                        ...
                        with self.mgr.async_timeout_handler(host, 'cephadm write-custom-logrotate-config'):
                            r = self.mgr.wait_async(self._write_custom_logrotate_config(
                                host, str(self.mgr.get_store(f'{str(logrotate_type)}_logrotate_config')), logrotate_type))
                        if r:
                            bad_hosts.append(r)
