...
class ExportMgr:
        ...
        try:
            ...
            if export:
                ...
                if export.fsal.name == NFS_GANESHA_SUPPORTED_FSALS[0]:
                    exports_count = self.get_export_count_with_same_fsal(export.fsal.cmount_path,
                                                                         cluster_id, export.fsal.fs_name)
                    if exports_count == 1:
    ...
    def create_export_from_dict(self,
        ...
        if fsal_type == NFS_GANESHA_SUPPORTED_FSALS[1]:
            ...
            if "cmount_path" in fsal and fsal["cmount_path"] != "/":
                _validate_cmount_path(self.mgr, fsal["cmount_path"], path)
    ...
    def create_cephfs_export(self,
        ...
        validate_cephfs_path(self.mgr, fs_name, path)
        if cmount_path != "/":
            _validate_cmount_path(self.mgr, cmount_path, path)
    ...
    def get_export_count_with_same_fsal(self, cmount_path, cluster_id, fs_name) -> int:
...
def generate_unique_id(cluster_id, fs_name, cmount_path):
    """
