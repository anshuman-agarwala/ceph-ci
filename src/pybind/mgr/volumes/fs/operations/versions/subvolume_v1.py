...
class SubvolumeV1(SubvolumeBase, SubvolumeTemplate):
    ...
    def get_pending_clones(self, snapname):
        ...
        try:
            if self.has_pending_clones(snapname):
                pending_track_id_list = self.metadata_mgr.keys_with_value_from_section('clone snaps', snapname)
            else:
        ...
        for track_id in pending_track_id_list:
            try:
                ...
                if e.errno != errno.ENOENT:
                    ...
                    try:
                        # If clone is completed between 'keys_with_value_from_section'
    ...
    def check_dangling_clone_index(self, snapname):
        ...
        try:
            ...
            log.error(f"failed to open clone index '{e}' for snapshot '{snapname}'")
            raise VolumeException(-errno.EINVAL, "failed to open clone index")
