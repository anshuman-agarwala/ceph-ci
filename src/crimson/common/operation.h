...
private:
  ...
  public:
	...
	PLOG(phase, trigger.get_op().get_id(), "OrderedConcurrentPhaseT::ExitBarrier::exit wait unlocking");
        assert(phase->core == seastar::this_shard_id());
        phase->mutex.unlock();
        phase = nullptr;
      }
...
public:
  ...
  PipelineExitBarrierI::Ref enter(TriggerT& t) {
    ...
    return std::make_unique<ExitBarrier<TriggerT>>(
      *this,
      mutex.lock().then([this, &t] {
	PLOG(this, t.get_op().get_id(), "OrderedConcurrentPhaseT::enter lock future resolved");
      }),
      t);
  }
