  ...
  std::set<std::pair<spg_t,epoch_t>> *split_pgs)
{
  assert_core();
  auto &pgs = pg_map.get_pgs();
  //PerShardState::osdmap should be the new one here? 
  return seastar::parallel_for_each(
    pgs.begin(), pgs.end(),
    [=, &shard_services] (auto &pg) {
...
seastar::future<> PerShardState::prime_splits(
  ShardServices &shard_services,
  epoch_t epoch,
  std::set<std::pair<spg_t, epoch_t>> pgids)
{
  LOG_PREFIX(PerShardState::prime_splits);
  INFO("IN PRIME SPLITS");
  ...
  bool do_create)
{
  ...
  auto get_pool_info_for_pg = [create_map, pgid, this] {
    ...
    DEBUG(" map epoch = {} for {}", create_map->get_epoch(), pgid);
...
void ShardServices::split_stats(Ref<PG> parent,
                 std::set<Ref<PG>> childpgs,
                 const std::set<spg_t> &childpgids,
                 PeeringCtx &rctx)
{
  LOG_PREFIX(ShardServices::split_stats);
  ...
  for (std::set<Ref<PG>>::const_iterator iter = childpgs.begin();
       iter != childpgs.end();
       ++iter, ++stat_iter) {
                                ...
                                PeeringCtx &rctx) 
{
  ...
  (auto& child) {
    ...
    return pg_to_shard_mapping.get_or_create_pg_mapping(child).then([this, child] (auto core) {
      ...
      return seastar::now();
    }).then([this, child, nextmap=nextmap] {
      LOG_PREFIX(ShardServices::split_pgs);
      DEBUG(" {} map epoch: {}", child.pgid, nextmap->get_epoch());
      return make_pg(std::move(nextmap), child, true);
    }).then([this, &parent, &out_pgs, &rctx, &pg_num](
    Ref<PG> child_pg) {
        LOG_PREFIX(ShardServices::split_pgs);
        ...
        DEBUG(" pg num is {}, m_seed is {}, split bits is {}", pg_num, child_pg->get_pgid().ps(), split_bits);
  ...
  }).then([this, &out_pgs, &nextmap, &rctx, &parent, &childpgids] {
    LOG_PREFIX(ShardServices::split_pgs);
    DEBUG(" before return");
    DEBUG(" OUT PGs: {}", out_pgs.size());
    split_stats(parent, out_pgs, childpgids, rctx);
    ...
    std::set<std::pair<spg_t,epoch_t>> *split_children)
{
  ...
  while (!queue.empty()) {
    ...
    if ((int)pgnum < new_pgnum) {
      // splitting
      if (cur.ps() < pgnum) {
        // We check all old pgs for children 
        std::set<spg_t> children;
	//seastar::logger().info("{} CUR PS: {} PGNUM: {}", __func__, cur.ps(), pgnum);
        if (cur.is_split(pgnum, new_pgnum,  &children)) {
	  //seastar::logger().info("{}  pg_num: {} -> {}", __func__, pgnum, new_pgnum);
