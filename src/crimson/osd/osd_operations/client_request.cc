  ...
  Ref<PG> pgref, const unsigned this_instance_id, instance_handle_t &ihref)
{
  ...
  if (pgref->last_completed_tid >= get_id()) {
    ERRORDPP("{}.{}: complete in wrong order, last_completed is {}",
             *pgref, *this, this_instance_id, pgref->last_completed_tid);
    ceph_abort();
  } else {
    pgref->last_completed_tid = get_id();
  }
}
