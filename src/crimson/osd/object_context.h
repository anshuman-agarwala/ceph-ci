...
private:
  ...
  template <typename Lock, typename Func>
  auto _with_promotion(Lock &lock, Func &&func) {
    Ref obc = this;
    lock.promote();
    return seastar::futurize_invoke(func).finally([&lock, obc] {
      lock.demote();
...
public:
  ...
  auto with_promoted_lock(Func&& func) {
    if constexpr (!std::is_void_v<InterruptCond>) {
      ...
      switch (Type) {
      case RWState::RWWRITE:
	return _with_promotion(lock.excl_from_write(), std::move(wrapper));
      case RWState::RWREAD:
	return _with_promotion(lock.excl_from_read(), std::move(wrapper));
      case RWState::RWEXCL:
	return seastar::futurize_invoke(std::move(wrapper));
      case RWState::RWNONE:
	return _with_lock(lock.for_excl(), std::move(wrapper));
	...
	assert(0 == "noop");
      ...
      switch (Type) {
      case RWState::RWWRITE:
	return _with_promotion(lock.excl_from_write(), std::forward<Func>(func));
      case RWState::RWREAD:
	return _with_promotion(lock.excl_from_read(), std::forward<Func>(func));
      case RWState::RWEXCL:
	return seastar::futurize_invoke(std::forward<Func>(func));
      case RWState::RWNONE:
	return _with_lock(lock.for_excl(), std::forward<Func>(func));
