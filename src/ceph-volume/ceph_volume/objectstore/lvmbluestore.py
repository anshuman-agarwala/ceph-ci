...
class LvmBlueStore(BlueStore):
    ...
    def _activate(self,
        ...
        if is_encrypted:
            ...
            #     real_path_device: str = os.path.realpath(lv_path)
            #     if real_path_device in disk.get_block_device_holders().keys() and \
            #         real_path_device in disk.get_block_device_holders().values():
            #             for k, v in disk.get_block_device_holders().items():
            #                 if v == real_path_device:
            #                     osd_lv_path = disk.get_lvm_mapper_path_from_dm(k)
            if disk.has_holders(lv_path):
