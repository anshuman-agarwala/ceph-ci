...
def _dd(device: str, count: int, skip: int = 0) -> str:
    ...
    try:
        with open(device, 'rb') as b:
            b.seek(skip)  # skip the first 4096 bytes
...
def get_bluestore_header(device: str) -> Dict[str, Any]:
    ...
    Raises:
        RuntimeError: If the specified device does not exist.
    ...
    if not os.path.exists(device):
        raise RuntimeError(f"{device} doesn't exist")

    out, err, rc = process.call([
        'ceph-bluestore-tool', 'show-label',
        '--dev', device], verbose_on_failure=False)
    if rc:
        logger.debug('device {} is not BlueStore; ceph-bluestore-tool failed to get info from device: {}\n{}'.format(device, out, err))
    else:
