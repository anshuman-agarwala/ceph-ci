...
from .disk import lsblk, device_family, get_part_entry_type, _dd
...
class CephLuks2:
    ...
    def config_luks2(self, config: Dict[str, str]) -> None:
        ...
        """
        assert 0 < len(config) <= 2, f'Invalid config for LUKS2 device {self.device}'
        assert all(key in ['label', 'subsystem'] for key in config.keys()), \
            f'LUKS2 config for device {self.device} can only be "label" and/or "subsystem".'
    ...
    def get_json_area(self) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {}
        try:
            data: str = _dd(self.device, 12288, 4096)
