...
class TestCephLuks2:
    ...
    def test_has_luks2_signature(self) -> None:
        with patch('ceph_volume.util.encryption._dd', return_value='LUKS'):
            assert encryption.CephLuks2('/dev/foo').has_luks2_signature

    @patch('ceph_volume.util.encryption._dd', side_effect=Exception('foo'))
    def test_has_luks2_signature_raises_exception(self, m_dd: Any) -> None:
    ...
    def test_get_label(self) -> None:
        with patch('ceph_volume.util.encryption._dd', return_value='fake-luks2-label'):
    ...
    def test_get_label_raises_exception(self) -> None:
        with patch('ceph_volume.util.encryption._dd', side_effect=Exception('fake-error')):
            with pytest.raises(RuntimeError):
    ...
    def test_get_subsystem(self) -> None:
        with patch('ceph_volume.util.encryption._dd', return_value='fake-luks2-subsystem'):
