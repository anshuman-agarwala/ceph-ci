...
int NVMeofGwMonitorClient::init()
{
  ...
  dout(4) << "monc.authentication done" << dendl;
...
static bool get_gw_state(const char* desc, const std::map<NvmeGroupKey, NvmeGwMonClientStates>& m, const NvmeGroupKey& group_key, const NvmeGwId& gw_id, NvmeGwClientState& out)
{
  ...
  if (gw_group == m.end()) {
    dout(4) << "can not find group (" << group_key.first << "," << group_key.second << ") "  << desc << " map: " << m << dendl;
    return false;
  }
  auto gw_state = gw_group->second.find(gw_id);
  if (gw_state == gw_group->second.end()) {
    dout(4) << "can not find gw id: " << gw_id << " in " << desc << "group: " << gw_group->second  << dendl;
    return false;
  }
  out = gw_state->second;
  return true;
...
void NVMeofGwMonitorClient::handle_nvmeof_gw_map(ceph::ref_t<MNVMeofGwMap> nmap)
{
  ...
  auto group_key = std::make_pair(pool, group);
  dout(4) << "handle nvmeof gw map: " << new_map << dendl;
  ...
  if (!got_old_gw_state) {
    if (!got_new_gw_state) {
      dout(4) << "Can not find new gw state" << dendl;
      return;
    }
    ...
    while (!set_group_id) {
      ...
      dout(4) << "GRPC set_group_id: " <<  new_gw_state.group_id << dendl;
      set_group_id = monitor_group_client.set_group_id( new_gw_state.group_id);
      if (!set_group_id) {
	      dout(4) << "GRPC set_group_id failed" << dendl;
  ...
  if (got_old_gw_state && got_new_gw_state) {
    dout(4) << "got_old_gw_state: " << old_gw_state << "got_new_gw_state: " << new_gw_state << dendl;
  ...
  for (const auto& nqn_state_pair: new_gw_state.subsystems) {
	...
	initial_ana_state;
      ...
      dout(4) << " grpid " << (ana_grp_index + 1) << " state: " << new_gw_state << dendl;
    }
  ...
  if (ai.states_size()) {
    ...
    while (!set_ana_state) {
      ...
      if (!set_ana_state) {
	dout(4) << "GRPC set_ana_state failed" << dendl;
    ...
    if (max_blocklist_epoch > osdmap_epoch) {
      ...
      dout(4) << "Ready for blocklist osd map epoch: " << osdmap_epoch << dendl;
    }
  }
  map = new_map;
}
...
{
  std::lock_guard l(lock);
  dout(4) << "got map type " << m->get_type() << dendl;
