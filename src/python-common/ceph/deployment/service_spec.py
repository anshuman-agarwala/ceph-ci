...
class NvmeofServiceSpec(ServiceSpec):
    ...
    def validate(self) -> None:
        ...
        if (self.max_subsystems and self.max_subsystems <= 0):
            raise SpecValidationError("Max subsystems must be greater than zero")

        if (self.max_namespaces and self.max_namespaces <= 0):
            raise SpecValidationError("Max namespaces must be greater than zero")

        if (self.max_namespaces_per_subsystem and self.max_namespaces_per_subsystem <= 0):
            raise SpecValidationError("Max namespaces per subsystem must be greater than zero")

        if (self.max_hosts_per_subsystem and self.max_hosts_per_subsystem <= 0):
