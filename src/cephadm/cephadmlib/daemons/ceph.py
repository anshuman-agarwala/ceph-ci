...
from ..file_utils import make_run_dir, pathify, populate_files, makedirs, recursive_chown
...
class CephExporter(ContainerDaemonForm):
    ...
    def get_daemon_args(self) -> List[str]:
        ...
        if self.https_enabled:
            args.extend(['--cert-file', '/etc/certs/ceph-exporter.crt',
                         '--key-file', '/etc/certs/ceph-exporter.key'])
        return args
