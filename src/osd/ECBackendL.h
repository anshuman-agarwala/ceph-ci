...
namespace ECLegacy {
  ...
  public:
    ...
    const ECUtilL::stripe_info_t sinfo;
    // If true the legacy code has been disabled by user and must not
    // be switched back on again. It is mutable because its only paranoia
    // policing and is set by a function which could otherwise by const.
