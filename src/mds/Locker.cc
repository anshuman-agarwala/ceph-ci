			   ...
			   bool auth_pin_nonblocking)
{
  ...
  for (auto& [co, flags]: quiesce_op_flags) {
    ...
    if (flags & LockOp::XLOCK) {
      dout(20) << "injecting an exclusive quiesce lock for inode " << in << " under mdr: " << mdr << dendl;
      lov.add_xlock(&in->quiescelock, 0);
      injected_quiesce_lock = true;
    } else if (flags & LockOp::WRLOCK) {
      dout(20) << "injecting a shared quiesce lock for inode " << in << " under mdr: " << mdr << dendl;
  ...
  for (const auto &p : mustpin) {
    ...
    int err = 0;
    if (!object->can_auth_pin(&err, mdr->want_bypass_freezing())) {
      if (mdr->lock_cache) {
	CDir *dir;
