  ...
  const NvmeGwId &gw_id, const NvmeGroupKey& group_key)
{
  ...
  for (auto& [_, gw_state]: group_gws) {
    add_grp_id(gw_state, fsm_timers[group_key][gw_id], i);
  }
  ...
  const NvmeGwId &gw_id, const NvmeGroupKey& group_key)
{
  ...
  for (auto &[id, gw_state]: group_gws) {
    remove_grp_id(gw_state, fsm_timers[group_key][gw_id], gw_state.ana_grp_id);
  }
