...
bool NVMeofGwMon::preprocess_command(MonOpRequestRef op)
{
    ...
    dout(10) << "MonCommand : "<< prefix << " size of int " << sizeof(int) << dendl;
    ...
    if ( prefix == "nvme-gw show" ){
        ...
        if( map.created_gws[group_key].size()){
            ...
            for (auto& gw_created_pair: map.created_gws[group_key]) {
                ...
                if(st.availability != gw_availability_t::GW_DELETING){ // not show ana group of deleting gw in the list -
            ...
            uint16_t num_ns[MAX_SUPPORTED_ANA_GROUPS+1] = {0,};
            uint16_t total_ns = 0;
...
bool NVMeofGwMon::prepare_command(MonOpRequestRef op)
{
    ...
    if( prefix == "nvme-gw create" || prefix == "nvme-gw delete" ) {
        ...
        if(prefix == "nvme-gw create"){
            ...
            bool deleted = false;
            rc = pending_map.cfg_delete_gw(id, group_key, deleted);
             if(rc == -ENOENT){
                 ...
                 bool propose;
                 ceph_assert(deleted == false);
                 process_gw_down(id, group_key, propose); // Simulate  immediate Failover of this GW
             }
        }
...
void NVMeofGwMon::process_gw_down(const NvmeGwId &gw_id, const NvmeGroupKey& group_key, bool &propose_pending){
    LastBeacon lb = {gw_id, group_key};
...
bool NVMeofGwMon::prepare_beacon(MonOpRequestRef op){
    ...
    if (avail == gw_availability_t::GW_CREATED){
                                   ...
                                   // it means it did not perform "exit" after failover was set by NVMeofGWMon
        {
           ...
           else if (pending_map.created_gws[group_key][gw_id].availability == gw_availability_t::GW_DELETING)
               goto false_return; // not analysing and not sending ack to the  beacons of GW in state Deleting
        }
    }
    ...
    if( pending_map.created_gws[group_key][gw_id].last_gw_map_epoch_valid == false ){
      dout(1) <<  "map epoch of gw is not up-to-date " << gw_id << " epoch " << map.epoch << " beacon_epoch " << m->get_last_gwmap_epoch() <<  dendl;
        ...
        if(sub.size())
            pending_map.track_deleting_gws(group_key, sub, gw_deleted_propose);  // Calculate number NSs in the DB of Gw in state == DELETING
    }
