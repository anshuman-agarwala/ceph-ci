...
void NVMeofGwMon::encode_pending(MonitorDBStore::TransactionRef t) {
    ...
    //features = 1;
    pending_map.encode(bl, features);//1
    dout(4) << "is REEF: " << HAVE_FEATURE(mon.get_quorum_con_features(), SERVER_REEF)<< " " << mon.get_quorum_con_features() << dendl;
    dout(4) << "is SQUID: " << HAVE_FEATURE(mon.get_quorum_con_features(), SERVER_SQUID) << dendl;
...
void NVMeofGwMon::check_sub(Subscription *sub)
{
    ...
    {
      //auto vers_state = map.peer_addr_2_version.find(entity_addrvec_t(sub->session->con->get_peer_addr()));
      //if(vers_state != map.peer_addr_2_version.end()) {
        dout(10) << "Sending map to subscriber " << sub->session->con << " " << sub->session->con->get_peer_addr() /*<< " sub. version " << vers_state->second*/ << dendl;
        sub->session->con->send_message2(make_message<MNVMeofGwMap>(map));
      /*}
      else{
        dout(10) << "not Sending map to subscriber " << sub->session->con << " " << sub->session->con->get_peer_addr()  << dendl;
      }*/
