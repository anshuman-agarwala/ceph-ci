...
inline  void decode(
  ...
  dout(0) << "decode NvmeGwTimers version = " << struct_version << dendl;
  decode(size, bl);
  for (uint32_t i = 0; i <size; i ++) {
    ...
    if (struct_version == STRUCT_VERSION) {
      ...
      //dout(0) << "decode Old NvmeGwTimers version = " << struct_version << dendl;
      decode(tick, bl);
      decode(val,  bl);
      Tmdata tm;
      tm.timer_started = tick;
  ...
  for (auto& gw : gws) {
    ...
    if (version == STRUCT_VERSION) {
      ...
      for (int i = 0; i < MAX_SUPPORTED_ANA_GROUPS; i++) {
        ...
        encode((uint32_t)bl_data[i].is_failover, bl);
      }
    }
...
inline void decode(
  ...
  DECODE_START(STRUCT_VERSION, bl);
  struct_version = struct_v;
  dout(0) << "decode NvmeGwMonStates. struct_v: " << struct_version << dendl;
  decode(num_created_gws, bl);
  dout(0) << "decode NvmeGwMonStates. num gws  " << num_created_gws << dendl;
  ...
  for (uint32_t i = 0; i<num_created_gws; i++) {
    ...
    NvmeAnaGrpId ana_grp_id;
    decode(ana_grp_id, bl);
    dout(0) << "decode NvmeGwMonStates. GW-id " << gw_name << " ana grpid "<< ana_grp_id <<  dendl;
    anas[ana_grp_id] = 1;
    ...
    if (struct_version == STRUCT_VERSION) {
      ...
      for (uint32_t i = 0; i <MAX_SUPPORTED_ANA_GROUPS; i ++) {
        decode(sm_state, bl);
        dout(0) << "decode NvmeGwMonStates state: " << i << " " << sm_state << dendl;
    ...
    if (struct_version == STRUCT_VERSION) {
      ...
      for (uint32_t i=0; i<MAX_SUPPORTED_ANA_GROUPS; i++) {
        ...
        uint32_t is_failover;
        decode(osd_epoch,   bl);
        dout(0) << "decode osd epoch  " << osd_epoch << dendl;
        decode(is_failover, bl);
        dout(0) << "decode is-failover  " << is_failover << dendl;
        Blocklist_data blst((epoch_t)osd_epoch, (bool)is_failover);
...
inline void encode(
  ...
  ENCODE_START(version, 1, bl);
  dout(0) << "Start encode for " << version <<" " << (uint32_t)created_gws.size() << dendl;
...
inline void decode(std::map<entity_addrvec_t , uint32_t> &peer2ver,
  ceph::buffer::list::const_iterator &bl) {
  uint32_t num_gws;
  dout(0) << "Decode peer_2_addr " << dendl;
  DECODE_START(2, bl);
  decode(num_gws, bl);
  for (uint32_t i = 0; i < num_gws; i++) {
...
inline void decode(BeaconSubsystem& sub, ceph::buffer::list::const_iterator &bl) {
  DECODE_START(STRUCT_VERSION, bl);
  dout(0) << "decode BeaconSubsystems " << dendl;
  decode(sub.nqn, bl);
  uint32_t s;
  sub.listeners.clear();
  decode(s, bl);
