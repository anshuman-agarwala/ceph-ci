...
class Quiescer(ThrasherGreenlet):
    ...
    quiesce_timeout: [1..),default=60 :: maximum time in seconds to wait for the quiesce to succeed
    quiesce_factor: [0.005..0.5],default=0.35 :: the fraction of the total runtime we want the system quiesced
    min_quiesce: [1..),default=10 :: the minimum pause time in seconds
    max_quiesce: [1..),default=60 :: the maximum pause time in seconds
    initial_delay: [0..),default=15 :: the time in seconds before the first quiesce
    ...
    def do_quiesce(self, duration):
        ...
        self.logger.info(f"Successfully quiesced, set_id: {set_id}, quiesce took {elapsed} seconds. Will release after: {duration}")
    ...
    def stop(self):
        log.info(f'The quiescer is requested to stop, running cancel all')
