...
class TestMirroring(CephFSTestCase):
    ...
    def test_cephfs_mirror_sync_already_existing_snapshots(self):
        ...
        dir_name = 'd0'
        self.mount_a.run_shell(['mkdir', '-p', f'{dir_name}'])
        self.mount_a.write_n_mb(os.path.join(dir_name, 'file.1'), 1)

        # take a snapshot
        snap_a = "snap_a"
        self.mount_a.run_shell(['mkdir', f'{dir_name}/.snap/{snap_a}'])

        # make a change in the fs and take another snapshot
        self.mount_a.write_n_mb(os.path.join(dir_name, 'file.2'), 1)
        snap_b = "snap_b"
        self.mount_a.run_shell(['mkdir', f'{dir_name}/.snap/{snap_b}'])

        # make another change in the fs and don't take snapshot
        self.mount_a.write_n_mb(os.path.join(dir_name, 'file.3'), 1)
